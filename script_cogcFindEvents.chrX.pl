...
my $pwd = `pwd`;
chomp($pwd);
my $refLoc = "./parentalData";
my $parentLoc = "./parentalData";
my $outputFile = "out_uniqueParentalVariants.chrX.tsv";

## Command-line options
my %opts;
getopts('ehols:', \%opts); # options as above. Values in %opts

## Usage Output
if ($opts{'h'}) { 
        print "
	This script identifies CO and NCO events in child .vcf files from two known
	parental genotypes. Parental files must exist in a parentalData/ directory,
	chid data must exist in a offspring/ directory.

	Optional:

		-e If $outputFile already exists you can use it and not re-make each time.

		-o If you only want to populate (or re-populate) $outputFile.
...
my $parentA = "Oregon-R-parental";
my $parentB = "ISO-1-parental";
...
if (!$opts{'e'}) {  # -e flag is to use existing out_uniqueParentalVariants.tsv file
	...
	my(%skip,%diffVariants,%diffVariantCounts);
...
print "[".localtime(time)."] Opening $outputFile.n";
open INF,"$pwd/$outputFile" or die "Can't open $outputFile: $!";
while (<INF>) {
	my(@F) = split /\t/, $_;
	next unless $F[1] =~ /[0-9]/;
...
#print "$chr,$id,$vcfScore,$ref,$alt\tA:$parental{$parentA}{$chr}{$id}\tB:$parental{$parentB}{$chr}{$id}\ti\n";
		...
		#if ($type eq "hom" ) {
#print "$chr,$id,$vcfScore,$ref,$alt\tA:$parental{$parentA}{$chr}{$id}\tB:$parental{$parentB}{$chr}{$id}\t";
